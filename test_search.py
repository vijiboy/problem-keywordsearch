...
subgenus Hippotigris;the plains zebra, the Grévy's zebra and the mountain zebra;horses and donkeys;aims to breed zebras that are phenotypically similar to the quagga; Grévy's zebra and the mountain zebra """
        ...
        expectedAnswers = ["Grévy's zebra and the mountain zebra",
        ...
        #print('\n'.join([str(cnt)+': '+ ' '.join(sentence) for cnt,sentence in enumerate(sentencesList)]))
        wordSearchSet = BuildSearchSetFromSentenceLists(sentencesList)

        #questions = [questions[1]]
        for Question in questions:
            question = Question[0:-1].split()
            #print("### Question: " +  ' '.join(question))
            answerSentences = getKeywordSearchedSentences(
                sentencesList, wordSearchSet,
                keywords = question) # question mark, the last character is ignored
            ...
            actualAnswers.append(answerSubstring)
        #print('expectedAnswers: ' + '\n'.join(expectedAnswers))
        #print('actualAnswers: ' + '\n'.join(actualAnswers))
        for i in range(len(questions)):
            self.assertEqual(expectedAnswers[i], actualAnswers[i])
